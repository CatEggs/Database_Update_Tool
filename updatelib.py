...
def update_df(df1, df2, id_lst, col1, col2, bkup_col,id1, id2):
        ...
        for id in id_lst:
                ...
                list_values = df2[df2[id2] == id][col2].values
                list_bkup_values = df2[df2[id2] == id][bkup_col].values
                df2_columnval = list(map(str, list_values))
                df2_columnval_bkup = list(map(str, list_bkup_values))
                df2_columnval = df2_columnval if all(df2_columnval)  != 'No change, no issue - resolved, Q Mismatch, but HB is good' else df2_columnval_bkup
